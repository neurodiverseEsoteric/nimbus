...
(function () {
 ...
 var BUTTON_TEXT={'ar':'تنزيل','cs':'Stáhnout','de':'Herunterladen','en':'Download','es':'Descargar','fr':'Télécharger','hi':'डाउनलोड','id':'Unduh','it':'Scarica','ja':'ダウンロード','pl':'Pobierz','pt':'Baixar','ro':'Descărcați','ru':'Скачать','tr':'İndir','zh':'下载'};
  ...
  var DECODE_RULE={};
  var RANDOM=23452241; // Math.floor(Math.random()*1234567890);
...
function start() {  
  ...
  if (isAjax && content) { // Ajax UI    
      ...
      } else { // MutationObserver fallback for Firefox<14, Opera<15, IE
...
function run() {
  ...
  if (videoID==null || videoTicket==null) { // unsafeWindow workaround (Chrome, Opera 15+)
    ...
    injectScript ('document.getElementById("'+DEBUG_ID+'2").innerHTML=\'"video_id":"\'+ytplayer.config.args.video_id+\'", "t":"\'+ytplayer.config.args.t+\'", "js":"\'+ytplayer.config.assets.js+\'", "url_encoded_fmt_stream_map":"\'+ytplayer.config.args.url_encoded_fmt_stream_map+\'"\';');            
    ...
    if (code!=null) {
      ...
      var scriptURLMatches=code.match(/\"js\":\s*\"([^\"]+)\"/);
  ...
  function getPref(name) { // cross-browser GM_getValue
    ...
    (typeof GM_getValue.toString === 'undefined' || GM_getValue.toString().indexOf('not supported') === -1)) {
      ...
      return localStorage.getItem(name); // Chrome script, Opera extensions
    } else return;
  }
  ...
  function setPref(name, value) { //  cross-browser GM_setValue
    ...
    (typeof GM_setValue.toString === 'undefined' || GM_setValue.toString().indexOf('not supported') === -1)) {
      ...
      localStorage.setItem(name, value); // Chrome script, Opera extensions
    }
  }
