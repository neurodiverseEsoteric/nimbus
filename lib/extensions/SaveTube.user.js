	...
	if (ytVideoCode) {
	  ...
	  if (myVideoCode) {
	    ...
	    if (ytVideo.match(/&sig=/)) ytVideo = ytVideo.replace (/&sig=/, '&signature=');
	      ...
	      if (ytSig) {
		var s = ytSig[1].split('');
		s = s.slice(35, 84).reverse().join('')+ s[0] + s.slice(28, 34).reverse().join('') + s[3] + s.slice(20, 27).reverse().join('') +
		    s[34] + s.slice(6, 19).reverse().join('') + s.slice(1, 3).reverse().join('') + s[27];
